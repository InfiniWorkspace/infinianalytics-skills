...
No dedicated node — the workflow calls the REST API from HTTP Request nodes at each lifecycle stage. Read [rest-api.md](rest-api.md) for the endpoint, headers, and body; this file covers the n8n wiring.
...
2. **Execution ID** — at the start of the workflow, a Set node (name it `execution_id`) with field `execution_id` set to:
